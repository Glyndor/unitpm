#!/usr/bin/env bash
# End-to-end smoke for unitpm / unitpmd. Runs against an already-installed
# CLI + daemon (system path or PATH override). The daemon is expected to
# be up and listening — the caller starts unitpmd beforehand.
#
# Intended callers:
...
die() {
    echo "FAIL: $*"
    [ -f /tmp/unitpmd.log ] && { echo "--- unitpmd.log tail ---"; tail -40 /tmp/unitpmd.log; }
    exit 1
}
...
run_worker_scenario() {
    ...
    unitpm logs "$1" --stdout --lines 10 2>/dev/null | grep -q "$3 pid=" || \
        die "$3 never printed its startup line"
    unitpm stop   "$1"
    unitpm delete "$1" --purge
}

# wait_count <expected> <selector-ns>
# Poll unitpm list until the number of procs in the given namespace
...
unitpm list --json >/dev/null || die "daemon socket never became responsive"

APPS_DIR="$(cd "$(dirname "$0")/apps" && pwd)"

# Sanity: empty list after a fresh daemon.
[ "$(unitpm list --json)" = "[]" ] || die "fresh daemon should have an empty process list"
...
unitpm start "/bin/sleep 300" --name smoke-vanilla --restart never
unitpm list --json | grep -q smoke-vanilla || die "smoke-vanilla not in list"
unitpm show smoke-vanilla >/dev/null
unitpm stop   smoke-vanilla
unitpm delete smoke-vanilla
[ "$(unitpm list --json)" = "[]" ] || die "list not empty after delete"
...
unitpm start "bash $APPS_DIR/shell-forkstorm/run.sh" --name fs --restart never
sleep 1
...
unitpm start "python3 $APPS_DIR/python-worker/worker.py" --name pyw --restart on-failure
sleep 1
unitpm restart pyw --json | grep -q '"op":"restart"' || die "restart --json missing op field"
unitpm reset   pyw --json | grep -q '"op":"reset"'   || die "reset --json missing op field"
unitpm flush   pyw --json | grep -q '"op":"flush"'   || die "flush --json missing op field"
unitpm stop    pyw
unitpm delete  pyw --purge
...
unitpm start "/bin/sleep 300" --name api    --namespace probe --restart never
unitpm start "/bin/sleep 300" --name worker --namespace probe --restart never
wait_count 2 probe
unitpm stop --namespace probe >/dev/null
# Both should now be stopped — list still shows them (stopped), delete
# with ns:* glob cleans them up in one shot.
unitpm delete 'probe:*' --purge >/dev/null
[ "$(unitpm list --namespace probe --json)" = "[]" ] || \
...
if command -v node >/dev/null; then
    ...
    unitpm start "node $APPS_DIR/node-ignores-term/server.js" \
        --name stubborn --restart never \
        --stop-signal SIGTERM --stop-timeout 2000
    sleep 1
    START=$(date +%s)
    unitpm stop stubborn
    END=$(date +%s)
    ELAPSED=$((END - START))
...
unitpm start "/bin/sleep 300" --name scaleapp --namespace scalens \
    --restart never --scale 3
wait_count 3 scalens
unitpm scale scalens:scaleapp 1
wait_count 1 scalens
unitpm scale scalens:scaleapp 2
wait_count 2 scalens
unitpm delete 'scalens:*' --purge >/dev/null

# Scenario 12: process tree. Starts a bash wrapper that spawns sleep children
# and verifies that unitpm monit --json reports the root entry and at least
...
unitpm stop   tree-smoke
unitpm delete tree-smoke --purge
