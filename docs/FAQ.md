...
|---------|--------|---------|
| Start and forget | ✅ | `lynx start app.js --restart always` |
| Stop all in a namespace | ⚠️ | no `stop --all`; use: `lynx list --json \| jq -r '.[] \| .name' \| xargs lynx stop` |
| Restart several at once | ✅ | `lynx restart a b c` |
| Reload spec without stopping process | ❌ | `lynx reload` does stop+start; no hot-reload of spec |
| Send custom signal | ❌ | only `--stop-signal` for stop; use `kill -USR1 $(pidof app)` |
| Scale without restarting | ✅ | `lynx scale app 5` (respects running instances) |
| Scale down to 0 | ✅ | `lynx scale app 0` = equivalent delete all |
| Reset `Restarts` counter | ✅ | `lynx reset app` |
...
| HTTP health check probe | ❌ | removed due to SSRF — use sidecar: `lynx start "curl -sSf http://localhost/h \|\| exit 1" --cron '@every 10s' --shell` |
...
| View env of a live process | ⚠️ | `lynx show <name>` shows spec; real env in `/proc/<pid>/environ` |
...
| stdout only | ✅ | `lynx logs api --stdout` |
| stderr only | ✅ | `lynx logs api --stderr` |
| Last N lines | ✅ | `lynx logs api --lines 50` |
| JSON-formatted logs | ✅ | `--log-format json` at `start` |
| Automatic rotation | ✅ | 50 MiB default, 3 backups (tunable env) |
| Truncate logs | ✅ | `lynx flush api` |
...
| Export running state → YAML | ✅ | `lynx export --namespace prod > apps.yml` |
...
| Parseable JSON output | ✅ | `lynx list --json` / `lynx version --json` |
| Shell completion | ✅ | `lynx completion bash\|zsh\|fish` |
| Namespace:name syntax | ✅ | `lynx show prod:api` |
| Resolve by ID prefix | ✅ | `lynx show 019d9` (if unique) |
| Multiple lifecycle commands in 1 cmd | ✅ | `lynx stop a b c d` |
...
| Erlang / Elixir | ✅ |
| Bash scripts | ✅ |
| Docker container | ⚠️ | yes via `docker run`, but lynx sandbox redundant |
...
| Can I…? | Yes/No | How |
|---------|--------|-----|
| Auto-start on boot | ✅ | `sudo lynx startup` (systemd) |
| Restore specs after reboot | ✅ | automatic on daemon start |
| Backup state | ✅ | copy `~/.config/lynx/apps/*.json` |
| Migrate between hosts | ✅ | `lynx export` → copy YAML → `lynx apply` |
...
| `lynx attach` / interactive stdin | no `docker exec`-style |
| Prometheus metrics endpoint | Parse `lynx list --json` from your scraper |
| Watch file mode (`--watch`) | Use nodemon/cargo-watch as sidecar |
| Deploy via SSH | Use Ansible / Terraform / rsync + `lynx apply` |
...
|-------|---------------|-----|
| `cannot reach the Lynx daemon` | daemon off | `lynxd &` (user) or `sudo systemctl start lynx.lynxd` (system) |
...
| `fork/exec: executable not found` | binary not in daemon PATH | `lynx install-tools` |
