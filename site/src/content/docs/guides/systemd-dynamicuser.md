...
description: Use systemd DynamicUser to run Linux services as unprivileged, isolated users with no persistent identity. How unitpm integrates DynamicUser sandboxing for Node.js, Python, and Go services.
---
...
This is one of the key security features unitpm exposes — PM2 and Supervisor have no equivalent.
...
DynamicUser pairs naturally with these systemd directives (all managed by unitpm with `--sandbox`):
...
## Enable DynamicUser with unitpm
...
systemctl show my-api.service | grep -E 'DynamicUser|PrivateTmp|ProtectSystem'
```
...
unitpm start "node server.js" \
  ...
  --cache-dir /var/cache/my-api
```

Or in systemd terms, `StateDirectory=my-api` creates `/var/lib/my-api` owned by the dynamic UID, and systemd reassigns ownership on each start. Your app can write there safely.
...
processes:
  api:
    command: node server.js
    dynamic_user: true
    state_directory: my-api
    cache_directory: my-api
```

Accessible at runtime as `/var/lib/my-api` and `/var/cache/my-api`.
...
| Write to `/var/lib/my-api` (state dir) | Allowed |
...
```bash
# IPv4 only
unitpm start "node server.js" --name api --sandbox --address-families inet
```

## Landlock filesystem restriction

unitpm also supports Linux 5.13+ Landlock for fine-grained filesystem access control:
...
unitpm start "node server.js" \
  ...
  --landlock-allow /var/lib/my-api:rwx \
...
…
→ Overall exposure level for my-api.service: 1.6 OK 🙂
```

A unitpm-managed service with `--sandbox` typically scores below 2.0 (excellent). A plain PM2 or Supervisor setup with no hardening scores 9.0+ (dangerous).

## Comparison with PM2 and Supervisor

| | unitpm (--sandbox) | PM2 | Supervisor |
|--|---------|-----|-----------|
| DynamicUser | ✓ | ✗ | ✗ |
| PrivateTmp | ✓ | ✗ | ✗ |
| NoNewPrivileges | ✓ | ✗ | ✗ |
...
PM2 and Supervisor run your process as whatever user you invoke them with. If that user is `root` (common in many tutorials), your application has full root access. unitpm's `--sandbox` enforces privilege drop at the systemd level — it cannot be bypassed by the application.
...
- [unitpm start](../reference/commands/start/) — `--sandbox`, `--dynamic-user`, `--landlock-allow` flags
...
- [unitpm vs PM2](./vs-pm2/) — security comparison
