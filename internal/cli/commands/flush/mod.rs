//! The `flush` command.
//!
//! 11 cases ported from `internal/cli/commands/flush/cmd_test.go`.
//!
//! Truncates one or more process's log files via the daemon's `flush`
//! verb. Targets may be literal IDs, namespace-qualified names, or the
//! `*` / `<ns>:*` wildcards. The daemon returns a `bytes_freed` count
//! which the CLI surfaces in both the human and `--json` outputs.

use std::collections::BTreeMap;
use std::io::Write;

use serde_json::json;

use crate::cli::batch::Report;
use crate::cli::errs::UsageError;
use crate::cli::expand::{self, ExpandError, ListClient, ListError};
use crate::cli::format;
use crate::cli::help::CommandSpec;
use crate::ipc::transport::{Client, IPCClient, TransportError};
use crate::term;
use crate::types::ProcessInfo;

/// Dyn-compatible IPC surface for the `flush` command.
pub trait Ipc {
	fn list(&mut self) -> Result<Vec<ProcessInfo>, TransportError>;
	fn flush(&mut self, id: &str) -> Result<FlushResponse, TransportError>;
}

/// Response shape returned by the daemon for a `flush` call.
#[derive(Debug, Clone, Default)]
pub struct FlushResponse {
	pub id: String,
	pub status: String,
	pub bytes_freed: i64,
}

/// Production adapter.
pub struct RealIpc(pub Client);

/// Adapter that lets [`RealIpc`] satisfy the [`ListClient`] surface used
/// by [`expand::targets`].
pub struct IpcList<'a>(pub &'a mut Box<dyn Ipc>);

impl<'a> ListClient for IpcList<'a> {
	fn list_processes(&mut self) -> Result<Vec<ProcessInfo>, ListError> {
		(**self.0)
			.list()
			.map_err(|e| ListError::Protocol(e.to_string()))
	}
}

impl Ipc for RealIpc {
	fn list(&mut self) -> Result<Vec<ProcessInfo>, TransportError> {
		self.0.list()
	}

	fn flush(&mut self, id: &str) -> Result<FlushResponse, TransportError> {
		self.0.flush(id)
	}
}

impl Ipc for Client {
	fn list(&mut self) -> Result<Vec<ProcessInfo>, TransportError> {
		self.call("list", None::<&()>, Some(&mut Vec::<ProcessInfo>::new()))?;
		let mut out: Vec<ProcessInfo> = Vec::new();
		self.call("list", None::<&()>, Some(&mut out))?;
		Ok(out)
	}

	fn flush(&mut self, id: &str) -> Result<FlushResponse, TransportError> {
		let body = serde_json::json!({"id": id});
		let mut val: serde_json::Value = serde_json::json!({});
		self.call("flush", Some(&body), Some(&mut val))?;
		let mut resp = FlushResponse::default();
		if let serde_json::Value::Object(map) = val {
			if let Some(s) = map.get("status").and_then(|v| v.as_str()) {
				resp.status = s.to_string();
			}
			if let Some(s) = map.get("id").and_then(|v| v.as_str()) {
				resp.id = s.to_string();
			}
			if let Some(n) = map.get("bytes_freed").and_then(|v| v.as_i64()) {
				resp.bytes_freed = n;
			}
		}
		Ok(resp)
	}
}

/// Run the `flush` command.
pub fn run<W: Write>(
	mut client: Option<Box<dyn Ipc>>,
	w: &mut W,
	args: &[String],
) -> Result<(), Box<dyn std::error::Error>> {
	if args.iter().any(|a| a == "-h" || a == "--help") {
		print_help(w);
		return Ok(());
	}

	let mut json_out = false;
	let mut namespace = String::new();
	let mut positional: Vec<String> = Vec::new();

	let mut i = 0;
	while i < args.len() {
		let a = &args[i];
		match a.as_str() {
			"--json" => json_out = true,
			"--namespace" => {
				if i + 1 >= args.len() {
					return Err(usage("missing value for --namespace".to_string()));
				}
				namespace = args[i + 1].clone();
				i += 1;
			}
			s if s.starts_with("--namespace=") => {
				namespace = s.trim_start_matches("--namespace=").to_string();
			}
			_ => positional.push(a.clone()),
		}
		i += 1;
	}

	if positional.is_empty() && namespace.is_empty() {
		return Err(usage("missing process ID or name".to_string()));
	}

	if client.is_none() {
		let c = Client::new()?;
		client = Some(Box::new(RealIpc(c)));
	}
	let client = client
		.as_mut()
		.expect("client either provided or opened above");

	let ids = expand_targets(client, &positional, &namespace)?;

	let mut rep = Report::new("flush");
	for id in &ids {
		match client.flush(id) {
			Ok(resp) => {
				if !json_out {
					if resp.bytes_freed > 0 {
						let _ = writeln!(
							w,
							"{} Flushed logs for {} {}",
							term::green(format_args!("{}", "✓")),
							resp.id,
							term::dim(format_args!("({} freed)", format::bytes(resp.bytes_freed)))
						);
					} else {
						let _ = writeln!(
							w,
							"{} Flushed logs for {}",
							term::green(format_args!("{}", "✓")),
							resp.id
						);
					}
				}
				let mut extra = BTreeMap::new();
				if resp.bytes_freed > 0 {
					extra.insert("bytes_freed".into(), json!(resp.bytes_freed));
				}
				rep.ok(&resp.id, extra);
			}
			Err(e) => {
				if !json_out {
					let _ = writeln!(
						w,
						"{} Failed to flush {}: {}",
						term::red(format_args!("{}", "✗")),
						id,
						e
					);
				}
				rep.fail(id, Some(&e));
			}
		}
	}

	if json_out {
		rep.emit_json_to(w).map_err(json_err)?;
	} else if rep.summary.total > 1 {
		rep.print_summary(w).map_err(json_err)?;
	}

	rep.err().map_or(Ok(()), |e| Err(Box::new(e)))
}

fn json_err(e: std::io::Error) -> Box<dyn std::error::Error> {
	Box::new(e)
}

fn usage(msg: String) -> Box<dyn std::error::Error> {
	Box::new(UsageError::new(msg))
}

fn expand_targets(
	client: &mut Box<dyn Ipc>,
	ids: &[String],
	namespace: &str,
) -> Result<Vec<String>, ExpandError> {
	let mut list = IpcList(client);
	expand::targets::<IpcList<'_>>(Some(&mut list), ids, namespace)
}

/// Help block for `--help`.
pub fn print_help<W: Write>(w: &mut W) {
	let _ = crate::cli::help::render_command_help(w, &spec());
}

/// Spec used by the registry / help renderer.
#[must_use]
pub fn spec() -> CommandSpec {
	CommandSpec {
		name: "flush".to_string(),
		aliases: Vec::new(),
		usage: "unitpm flush <id|name|ns:*|*>... [--namespace <ns>] [--json]".to_string(),
		description: "Flush logs for a process".to_string(),
		options: vec![
			crate::cli::help::Option {
				short: String::new(),
				long: "--namespace <ns>".to_string(),
				description: "Flush every process in this namespace.".to_string(),
			},
			crate::cli::help::Option {
				short: String::new(),
				long: "--json".to_string(),
				description: "Emit a machine-readable batch report.".to_string(),
			},
			crate::cli::help::Option {
				short: "-h".to_string(),
				long: "--help".to_string(),
				description: "Show this help message.".to_string(),
			},
		],
		examples: vec![
			"unitpm flush api".to_string(),
			"unitpm flush prod:api prod:worker".to_string(),
			"unitpm flush 'prod:*'        # every process in namespace prod (quote the glob)"
				.to_string(),
			"unitpm flush --namespace prod # equivalent, no shell quoting needed".to_string(),
			"unitpm flush api --json | jq '.results[].extra.bytes_freed'".to_string(),
		],
		hidden: false,
	}
}

#[cfg(test)]
mod tests;
